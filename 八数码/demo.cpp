...
void list_reverse(p_node &p);           //将表逆序排序  
...
void out_node(p_node s) //输出八数码  
{
	...
	printf(" 搜索深度d=%d ,与目标节点的不同棋子个数w=%d，总评估函数值f=%d\n\n\n", s->d, s->w, s->f);
...
void move(p_node s)//移动父节点并加入未探索表中（扩展结点）  
{
	...
	//并调用函数与表中已经存在的节点对照，不走重复过的走法，
	//防止出现围绕着几种固定走法进行遍历却无法达到目标棋局情况，防止出现死循环情况，  
